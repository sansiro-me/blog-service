<template>
    <div class="mdContainer" :class="{ fullPage: fullPageStatus }">
        ...
        <div class="upimg" v-show="isupimg">
          <div class="select-area">
            <h2 class="select-title">图片上传</h2>
            <div class="select-main">
              <p class="file-p">点击或拖拽至此处</p>
              <input type="file" name="file" class="file" @change="uploadImage" ref="select_area">
              <div class="show-area">
                <img :src="imgsrc" alt="" class="show-img">
              </div>
            </div>
            <div class="select-btn">
              <button class="select-btn-left" @click="insertImg">确定</button>
              <button @click="cancelUpImg">取消</button>
            </div>
          </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        mounted () {
          this.$refs.select_area.ondragleave = (e) => {
            e.preventDefault();
          };
          this.$refs.select_area.ondragenter = (e) => {
            e.preventDefault();
            this.$refs.select_area.borderColor = 'red';
          };
          this.$refs.select_area.ondragover = (e) => {
            e.preventDefault();    //阻止拖来拖去的浏览器默认行为
          };
          this.$refs.select_area.ondrop = (e) => {
            e.preventDefault();    //阻止拖放后的浏览器默认行为
            this.imgsrc = '/images/loading.gif';
            const file = e.dataTransfer.files[0];  // 获取文件对象
            if (file.length < 1) {
              return;  // 检测是否有文件拖拽到页面     
            }
        ...
        methods: {
            ...
            insertImg () {
              ...
              insertContent("![alt](" + this.imgsrc + ")", this);
              this.isupimg = false;
            },
            cancelUpImg () {
              this.isupimg = false;
...
<style lang="css">
    ...
    /*引入atom的代码高亮样式文件*/
    @import url("../../static/css/haha.css"); 
    @import url("../../static/css/new.css"); 
    ...
    .upimg .select-btn button {
      height: 50px;
      flex-grow: 1;
      outline: none;
      border: none;
    }
    .upimg .select-btn button:hover {
      background-color: #ededed;
    }
    .upimg .select-btn .select-btn-left {
      color: #ff6d6d;
    }
</style>
